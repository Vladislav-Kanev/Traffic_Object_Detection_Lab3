...
class Trainer:
    ...
    def eval(self) -> SubmissionType:
        self._model.eval()
        result = []
        for batch in tqdm(self._val_dataloader, desc='Validation'):
            batch_imgs = []
            for sample in batch:
                batch_imgs.append(sample[0].to(self._device))
            batch_result_dict = self._model(batch_imgs)
            result.extend(batch_result_dict)
        return result
